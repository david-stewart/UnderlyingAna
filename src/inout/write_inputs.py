...
class Words:
    def __init__(self, f_c, f_h, f_txt):
        self.words = []
        self.headings = []
        self.spaces = 4
        ...
        for entry in in_tuples:
           ...
           #HERE 
            ...
            if (ctype == 'llint'):
                ...
                self.fh.write(' '*self.spaces + 'std::string   ' + name + ';\n')
            else:
            ...
            typstr = '.'+ctype+'.'
            if len(entry) == 2:
                self.ftxt.write('    %-20s <!!> %-8s'%(name,typstr))
            else:
                self.ftxt.write('    %-20s %6s %-8s'%(name,entry[2],typstr))

            if len(entry) > 3:
                self.ftxt.write(' '+entry[3]+'\n')
                for E in tuple(entry[4:]):
                    self.ftxt.write(' '*34 + '// ' + E + '\n')
            else: self.ftxt.write('\n')

fh   = open('InputReader.h','w')
fc   = open('InputReader_c.h','w')
ftxt = open('user.i','w') #'run/RunJetFinding.input','w')
words = Words(fc,fh,ftxt)
